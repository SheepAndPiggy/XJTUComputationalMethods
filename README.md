...
此外，通过将斯蒂芬森加速技术应用于简单迭代法，可以明显观察到加速策略对收敛性能的改善作用。原本需要 122 步的迭代过程，在经过加速后仅用了 3 步便达到了与牛顿法相同的收敛精度，这说明斯蒂芬森加速在有效提升线性收敛方法的速度方面具有显著优势。其核心思想是利用多次迭代结果构造更高阶的收敛形式，从而在不改变原迭代函数结构的情况下增强收敛阶。尽管简单迭代法本身收敛较慢，但适当的加速技术能够显著提高其实际可用性。因此，在工程计算或函数结构难以满足牛顿法条件的场景中，结合加速策略的简单迭代法可能成为一种成本较低、实现灵活且具备较好收敛效果的替代方案。
